...
# Initialize Master Telegram Bot Client
bot = TelegramClient('ghost_ads_bot_advanced_session', MASTER_API_ID, MASTER_API_HASH)

# In-memory runtime maps
USER_DATA = {}          # Temporary wizard steps
ACTIVE_TASKS = {}       # Background running ad tasks
...
@bot.on(events.CallbackQuery(data=b"select_groups"))
async def select_groups_callback(event):
    ...
    await event.edit("⏳ Aapke account ke groups fetch kiye ja rahe hain, kripya intezaار karein...")
    
    try:
        # Load active client from saved database session string
...
@bot.on(events.CallbackQuery(pattern=b"toggle_g_"))
...
@bot.on(events.CallbackQuery(data=b"save_groups_selection"))
...
@bot.on(events.CallbackQuery(pattern=b"spd_"))
...
# --- RUN & STOP ADS ---
@bot.on(events.CallbackQuery(data=b"run_ads"))
...
# --- RENDER WEBHOOK PING SERVER (KEEP ALIVE FOR RENDER) ---
